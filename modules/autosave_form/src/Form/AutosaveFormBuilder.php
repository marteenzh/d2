...
class AutosaveFormBuilder extends FormBuilder {
  ...
  public function processForm($form_id, &$form, FormStateInterface &$form_state) {
    ...
    elseif ($this->isRestoreTriggered($form, $form_state)) {
      // We need this in order to let our override of ::rebuildForm run as
      // first and then the one of the decorated service.
      $process_by_decorator = TRUE;
      $triggering_element = NULL;
    }

    if ($process_by_decorator) {
      $response = parent::processForm($form_id, $form, $form_state);
      $this->doBuildFormSkip = FALSE;
    }
    else {
      $response = $this->formBuilder->processForm($form_id, $form, $form_state);
    }
  ...
  public function doBuildForm($form_id, &$element, FormStateInterface &$form_state) {
    return $this->doBuildFormSkip ? $element : $this->formBuilder->doBuildForm($form_id, $element, $form_state);
  }
