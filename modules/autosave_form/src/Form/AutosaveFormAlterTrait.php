...
trait AutosaveFormAlterTrait {
  ...
  public function formAlter(array &$form, FormStateInterface $form_state) {
    ...
    // thus leading to the underlying code being executed as well and putting
    // the autosave resume/discard message to the form, which should not happen
    // if the form is being returned to the browser with validation errors. In
    // order to prevent this we have to add the resume/discard message and
    // options only on HTTP GET requests or on POST requests if restore or
    // reject submit operations have been performed.
    $show_restore_discard_options = ($this->getRequestMethod() == 'GET') || $this->isRestoreTriggered($form, $form_state) || $this->isRejectTriggered($form, $form_state);
    if ($show_restore_discard_options && !$form_state->get('autosave_form_state_timestamp') && !$form_state->get('autosave_form_rejected') && ($autosave_form_state_timestamp = $this->getLastAutosavedTimestamp($form_state, $this->currentUser->id()))) {
  ...
  public function autosaveFormRejectSubmit($form, FormStateInterface $form_state) {
    ...
    $this->purgeAutosavedState($form_state, $this->currentUser->id());
  }
  ...
  public function autosaveFormSubmit($form, FormStateInterface $form_state) {
    ...
    foreach (['form_build_id', 'form_token', 'ajax_page_state', 'autosave_form_last_autosave_timestamp'] as $skip_from_comparision_key) {
      unset($autosaved_form_state_input[$skip_from_comparision_key], $form_state_input[$skip_from_comparision_key]);
    }
   ...
   */
  protected function getAutosaveFormSessionID(FormStateInterface $form_state) {
