...
class AutosaveEntityFormHandler implements AutosaveEntityFormHandlerInterface, EntityHandlerInterface {
  ...
  public function isAutosaveSubmitValid(FormStateInterface $form_state) {
    ...
    if ($entity instanceof EntityChangedInterface) {
      $unchanged = $this->entityStorage->loadUnchanged($entity->id());
      ...
      if ($unchanged->getChangedTimeAcrossTranslations() > $entity->getChangedTimeAcrossTranslations()) {
   ...
   * {@inheritdoc}
   */
  public function purgeAutosavedState(FormStateInterface $form_state, $uid) {
    list($form_id, $entity) = $this->getFormIDandEntity($form_state);
    $this->autosaveEntityFormStorage->purgeAutosavedEntityState($entity->getEntityTypeId(), $entity->id(), $form_id, $entity->language()->getId(), $uid);
  }
